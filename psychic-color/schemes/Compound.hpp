...
namespace psychic_color {
    ...
    constexpr std::size_t CompoundNumColors = 6;

    template<class T>
    class Compound : public ColorWheelScheme<T, CompoundNumColors> {
    public:
        explicit Compound(const T &primaryColor, bool ryb = false);
        void generate() override;
    protected:
    };

    template<class T>
    Compound<T>::Compound(const T &primaryColor, bool ryb):
        ColorWheelScheme<T, CompoundNumColors>(primaryColor, ryb) {
        generate();
    }

    template<class T>
    void Compound<T>::generate() {
